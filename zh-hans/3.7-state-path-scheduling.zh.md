                    ...
                    match result {
                        Ok(messages) => {
                            ...
                            for msg_record in messages {
                                // 1. 解码消息
                                let envelope = RpcEnvelope::decode(&msg_record.payload[..])?;
...
impl<W: Workload> ActrNode<W> {
    async fn handle_incoming(&self, envelope: RpcEnvelope) -> ActorResult<Bytes> {
        // 1. 创建 RuntimeContext（包含 caller_id, request_id）
...
// 注意：traceparent 和 tracestate 会通过 OpenTelemetry 自动传播
...
CREATE TABLE messages (
    ...
    payload BLOB NOT NULL,            -- RpcEnvelope (Protobuf bytes)
...
impl EchoService {
    async fn echo<C: Context>(&self, ctx: &C, req: EchoRequest) -> ActorResult<EchoResponse> {
        ...
        // 注意：trace_id 现在通过 OpenTelemetry span 自动管理
