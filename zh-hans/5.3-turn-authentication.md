...
sequenceDiagram
    ...
    alt 缓存命中
        ...
        opt 如果需要从 KS 获取密钥
            ...
            Note left of ks: secret_key: String<br/>expires_at: u64<br/>in_tolerance_period: bool
        end
    ...
    turn->>turn: 18. 验证租户
    Note right of turn: 验证 Realm 是否存在、未过期、状态正常
...
**实现位置** (`actrix/crates/turn/src/authenticator.rs`):

```rust
use actrix_common::realm::RealmEntity;

impl Authenticator {
    fn verify_realm(&self, token: &Token) -> Result<(), Error> {
        // 验证 Realm 是否存在、未过期、状态正常
        let realm_id = token.tenant_id;
        if let Err(e) = RealmEntity::validate_realm(realm_id).await {
            warn!("⚠️ TURN 认证 realm 验证失败: {}", e);
            return Err(Error::RealmValidationFailed(e));
        }
        Ok(())
    }
}
...
**错误响应**:
- **401 Unauthorized**: 当 Realm 验证失败时返回
- 错误信息包含具体的失败原因（不存在/过期/状态异常）
- 阻止 TURN 认证继续进行，避免无效的媒体中继
...
| 错误码 | 原因 | 处理方式 |
|--------|------|---------|
| **401 Unauthorized** | 认证失败（PSK 错误、Realm 验证失败） | 检查 PSK 是否正确、Realm 是否有效 |
