...
**关键设计**：
- **双向关联**：`Workload::Dispatcher` 和 `MessageDispatcher::Workload` 互相关联
...
```rust
// crates/framework/src/context.rs

/// Context - Actor 执行上下文接口（trait）
///
/// 定义 Actor 与系统交互的完整接口，由 Runtime 实现
#[async_trait]
pub trait Context: Send + Sync + Clone + 'static {
    ...
    // ... 更多方法（DataStream、MediaTrack 等）
}
```

**关键特性**：
- ✅ **接口定义**：Framework 只定义 trait，Runtime 实现（依赖倒置）
- ✅ **泛型参数**：用户代码使用 `<C: Context>` 而非 `&dyn Context`（静态分发）
- ✅ **Clone 友好**：实现 Clone，可以在异步任务间传递
- ✅ **零虚函数调用**：通过泛型单态化实现编译时优化
...
async fn call_remote_service<C: Context>(ctx: &C, target_id: &ActrId) -> ActorResult<String> {
