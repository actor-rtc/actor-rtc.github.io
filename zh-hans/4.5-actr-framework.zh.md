...
│    - 关联 Router 类型                                   │
│    - 提供 actor_type(), on_start(), on_stop()          │
...
- ✅ **上下文携带**：包含 trace_id、request_id 等追踪信息
...
impl Context {
    ...
    pub async fn call<R: RpcRequest>(
        ...
        // 2. 构造 RpcEnvelope（继承 trace_id，包含 route_key）
...
| 职责 | 说明 | 示例 API |
|------|------|----------|
| **上下文携带** | 携带当前调用的上下文信息 | `ctx.caller_id`<br/>`ctx.trace_id` |
...
**依赖说明**：
- `actr-protocol`：协议层（ActrId, MessageEnvelope, ActorResult 等）
...
|-------|------|---------------------|
| **actr-protocol** | 数据类型定义（ActrId, MessageEnvelope 等） | Framework 依赖 protocol |
