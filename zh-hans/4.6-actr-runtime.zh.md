...
| 模块 | 文件位置 | 职责 |
|------|---------|------|
| **Lane** | `transport/lane.rs` | **核心抽象：PayloadType 的物理体现，双向通道**<br/>• `send(Bytes)` / `send_envelope(MessageEnvelope)` - 出站写入<br/>• `recv()` / `recv_envelope()` - 入站读取<br/>• 类型：Mpsc, WebRtcDataChannel, WebRtcMediaTrack, WebSocket |
...
|---------|----------|-------------------|
| **Inproc (mpsc)** | **Lane::Mpsc** | 每个 PayloadType 一个 mpsc channel，**直接传递 MessageEnvelope 对象（零序列化）** |
...
| InprocOutGate | `outbound/inproc_out_gate.rs` | 进程内出站适配器，直接传递 MessageEnvelope |
...
**零序列化**：直接传递 MessageEnvelope 对象，无需 protobuf 编解码。
...
├─────────────────────────────────────────────────────────────┤
   InprocTransportManager.get_lane(payload_type, identifier)
      ...
      └─ 返回 Lane::Mpsc
      ↓
   Lane.send_envelope(envelope)  ← **直接传递 MessageEnvelope 对象**
      ↓
┌─────────────────────────────────────────────────────────────┐
│ 底层: tokio::sync::mpsc (无 Wire 层)                         │
├─────────────────────────────────────────────────────────────┤
   mpsc::Sender<MessageEnvelope>.send(envelope)
      ↓
   **零序列化：直接传递对象**
      ↓
   对端 InprocTransportManager.recv() → MessageEnvelope
└─────────────────────────────────────────────────────────────┘
```
...
**注意**: Inproc 入站路径**直接接收 MessageEnvelope 对象**，零序列化。
...
│ 底层: tokio::sync::mpsc (无 Wire 层)                         │
├─────────────────────────────────────────────────────────────┤
   mpsc::Receiver<MessageEnvelope>.recv()
      ↓
   **零序列化：直接接收 MessageEnvelope 对象**
      ↓
┌─────────────────────────────────────────────────────────────┐
│ Layer 1: Transport (InprocTransportManager)                  │
├─────────────────────────────────────────────────────────────┤
   InprocTransportManager.recv() → MessageEnvelope
      ↓
   tokio::select! { biased;
       msg = rx_signal.recv() => ...
   ...
   try_complete_response(envelope):
      if pending_requests.contains(request_id):
          ...
          return  // 已处理，不返回给调用方
      ↓
   返回 MessageEnvelope（如果不是 response）
      ↓
┌─────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────┤
   ...
   message_rx.recv() → (from: ActrId, data: Vec<u8>)
      ↓
   解码 MessageEnvelope::decode(data)
      ↓
┌─────────────────────────────────────────────────────────────┐
│ Layer 3: Inbound                                             │
├─────────────────────────────────────────────────────────────┤
...
├─────────────────────────────────────────────────────────────┤
   ...
   lane_dispatchers[payload_type].send(data)
      ↓
   【后续流程与 WebRTC 入站相同】
   解码 MessageEnvelope → Inbound → Application
└─────────────────────────────────────────────────────────────┘
```
...
| **序列化** | Protobuf (MessageEnvelope) | 零（直接 RTP） |
...
```rust
pub enum Lane {
    // Inproc: 直接传递 MessageEnvelope（零序列化）
    Mpsc {
        payload_type: PayloadType,
        tx: mpsc::Sender<MessageEnvelope>,
        rx: Arc<Mutex<mpsc::Receiver<MessageEnvelope>>>,
    },
...
**关键特性**：
- **双接口设计**：
  - Inproc: `send_envelope(MessageEnvelope)` / `recv_envelope()` - 零序列化
  - Outproc: `send(Bytes)` / `recv()` - 需序列化
- **零拷贝传输**：
  - Inproc: 直接传递 MessageEnvelope 对象
...
- Inproc 路径类似，但通过 Lane::Mpsc 直接传递 MessageEnvelope（零序列化）
...
- **Lane::Mpsc 零序列化**：直接传递 MessageEnvelope 对象，无 protobuf 编解码
