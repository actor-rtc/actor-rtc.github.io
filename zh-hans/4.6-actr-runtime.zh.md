...
│
├── inbound/            # Layer 3: 入站回调层
│   ├── mod.rs
│   ├── data_stream_registry.rs   # LatencyFirst/StreamReliable 回调注册
...
graph TB
    ...
    subgraph L3["Layer 3: Inbound 入站处理层"]
        WG["WebRtcGate<br/>跨进程入站路由"]
        INPROC["Inproc Receive Loops Shell↔Workload"]
        MB["Mailbox"]
        DSR["DataStreamRegistry"]
        MFR["MediaFrameRegistry"]

        WG -->|RpcReliable/RpcSignal| MB
        WG -->|StreamReliable/LatencyFirst| DSR
        INPROC -->|Shell→Workload 请求| WH
        MB -->|调度| WH
        DSR -->|回调| WH
        MFR -->|WebRTC Track 回调| WH
    end
    ...
    class WH app
    class WG,INPROC,MB,DSR,MFR router
    class OG gate
    class TM transport
    class LANE lane
...
### Layer 3: Inbound（入站处理层）

**职责**：在进程边界内完成消息解包、路由和回调

| 模块 | 文件位置 | 职责 |
|------|---------|------|
| WebRtcGate | `wire/webrtc/gate.rs` | 消费 `WebRtcCoordinator` 聚合的入站字节流，按 PayloadType 路由到 Mailbox 或 DataStreamRegistry，并处理 pending_requests |
| Inproc Receive Loops | `lifecycle/actr_node.rs` | 两个 tokio 循环直接从 `InprocTransportManager` 的 Lane::Mpsc 收包，实现 Shell↔Workload 请求/响应闭环 |
| DataStreamRegistry | `inbound/data_stream_registry.rs` | LatencyFirst/StreamReliable 类型消息的回调注册 |
| MediaFrameRegistry | `inbound/media_frame_registry.rs` | WebRTC MediaTrack 回调注册，直接挂在 PeerConnection |
...
stateDiagram-v2
    ...
    ActrSystem --> ActrNode: attach(workload)<br/>消费 self，引入泛型

    ActrNode --> ActrRef: start()<br/>消费 self，启动后台循环
    ...
    state ActrRef {
        ...
        state InboundLoops {
            ...
            WebRtcGate: WebRtcGate.start_receive_loop()
        }

        InprocMgr --> WorkloadHandlers
        WebRtcGate --> Mailbox
        Mailbox --> WorkloadHandlers
    }
...
- Inproc 循环直接调用 `handle_incoming()`，WebRtcGate 经 Mailbox/Registry 再回调 Workload
...
| **入站路由** | Shell→Workload/Shell←Workload 两个 Inproc 循环 | WebRtcGate + Mailbox/DataStreamRegistry |
| **序列化** | **零序列化（直接传递 MessageEnvelope 对象）** | Protobuf 序列化（bytes） |
...
**出站流向始终由 OutGate 直接进入 Transport（Inproc: Lane::Mpsc, Outproc: WebRTC/WebSocket）**。
...
**出站关键点**：
1. **无额外入站组件** - 直接从 Outbound 进入 Transport/Wire
...
> Inproc 入站不会经过 Mailbox；Shell→Workload 请求直接调用 Workload 处理逻辑，Workload→Shell 响应通过 `complete_response()` 唤醒等待方。
...
│ Layer 3: Inbound                                             │
├─────────────────────────────────────────────────────────────┤
   WebRtcGate.start_receive_loop()
      ↓
   match payload_type {
       RpcReliable/RpcSignal → decode RpcEnvelope
           → 若 pending_requests.contains(request_id) 则唤醒等待者
           → 否则根据优先级 mailbox.enqueue(from_bytes, raw_data)
       StreamReliable/LatencyFirst → decode DataStream
           → decode ActrId → DataStreamRegistry.dispatch(chunk, sender_id)
       MediaRtp → 回调应直接来自 RTCTrackRemote（此处仅记录告警）
   }
      ↓
   【State Path】Mailbox 路径:
      ...
      Workload.handle_xxx(request, ctx)
          ↓
      若 envelope 为响应 → OutprocOutGate.pending_requests 完成
      否则 → 处理请求并从 WebRtcGate.send_response() 返回
      ↓
   【Fast Path】DataStreamRegistry 路径:
      user_callback(StreamChunk, sender_id: ActrId)
          ↓
└─────────────────────────────────────────────────────────────┘
```

#### Outproc 入站（跨进程 - WebSocket，当前默认禁用）
...
├─────────────────────────────────────────────────────────────┤
   ...
   根据 PayloadType 路由到对应 Lane.rx
      ↓
   lane_dispatchers[payload_type].send(Bytes)
      ↓
   （需复用 WebRtcGate 的入站逻辑，当前 enable_websocket=false 尚未启用）
└─────────────────────────────────────────────────────────────┘
```

**入站关键点**：
1. **Inproc vs Outproc 入站差异**：
   - **Inproc**: InprocTransportManager 自动处理 pending_requests，只返回真正的 request；Shell→Workload 循环直接调用 Workload。
   - **Outproc**: Lane 将 Bytes 交给 WebRtcGate，WebRtcGate 决定是唤醒 pending request 还是入队 Mailbox/DataStreamRegistry。
2. **Wire 层接收** - 物理层接收机制：
   - **Inproc**:
...
4. **Inbound 层路由** - 按 PayloadType 路由到 Mailbox 或 Registry：
   - **Inproc**: 直接路由到 Workload（Shell → Workload），不经过 Mailbox。
   - **Outproc**: WebRtcGate 必须区分 RPC/Stream（Mailbox vs DataStreamRegistry）。
...
| **Inbound 路由** | WebRtcGate → Mailbox（或 Shell→Workload 直接处理） | MediaFrameRegistry → 直接回调 |
...
**问题**：
- State Path (RPC): 需要完整的 Context（request_id, caller_id, 出站能力）
- Fast Path (Stream): 只需要发送方标识（ActrId），不需要 RPC 语义
- 追踪上下文通过 OpenTelemetry 传播
...
sequenceDiagram
    ...
    L->>W: send(bytes)
    Note over W: WebRTC DataChannel 或 WebSocket 二进制帧
```
