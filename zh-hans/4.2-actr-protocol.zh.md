...
- ActrId 字符串表示（`actor_ext.rs`）
  - 格式：`<serial_number (16 进制)>@<realm_id>:<manufacturer>+<name>`
  - 例如：`1a2b3c@1001:acme+echo-service`

- URI 解析（`uri.rs`）
  - 格式：`actr://<realm>:<manufacturer>+<name>@<version>`
  - 例如：`actr://1001:acme+echo-service@v1`
...
assert_eq!(id.to_string_repr(), "1a2b3c@1001:acme+echo-service");
```
