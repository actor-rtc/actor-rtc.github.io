...
impl KeyServer {
    async fn get_secret_key(&self, key_id: u32) -> Result<GetSecretKeyResponse> {
        ...
        } else if now < key.expires_at + self.config.grace_period_secs {
            KeyStatus::GracePeriod  // 容忍期内
        } else {
            KeyStatus::Expired  // 完全过期
        };
        
        match status {
            KeyStatus::Active | KeyStatus::GracePeriod => {
...
KeyServerConfig {
    ...
    key_grace_period_secs: 3600,      // 1 小时
}
...
- `grace_period >= credential_ttl` （确保所有 credential 都能在容忍期内更新）
