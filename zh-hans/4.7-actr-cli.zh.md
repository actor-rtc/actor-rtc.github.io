...
| `actr gen` | 生成多语言 Actor 代码 | ConfigService<br/>CodegenService |
| `actr run [script]` | 执行自定义脚本 | ConfigService |
| `actr config <subcommand>` | 项目配置管理 | ConfigService |
| `actr fingerprint` | 计算服务语义指纹 | ConfigService |
| `actr check` | 配置和依赖验证 | ConfigService<br/>NetworkService |
| `actr discovery` | 服务发现 | NetworkService |
| `actr doc` | 生成项目文档 | FileService |
...
```mermaid
graph TD
    A[actr init name] --> B{检查目录是否存在}
    B -->|已存在| C[报错退出]
    B -->|不存在| D[创建项目目录]
    D --> E[生成 Actr.toml]
    E --> F[生成 proto/ 目录和示例文件]
    F --> G[生成 src/main.rs 模板]
    G --> H[生成 Cargo.toml]
    H --> I[生成 .gitignore]
    I --> J[完成]
```
...
```bash
# 默认生成 (Rust, 输入: protos, 输出: src/generated)
actr gen

# 指定语言
actr gen --language swift
actr gen --language kotlin

# 指定输入输出目录
actr gen --input ./my_protos --output ./out_dir

# 清理旧代码并重新生成
actr gen --clean

# 强制覆盖已有的用户代码脚手架 (慎用)
actr gen --overwrite-user-code

# 仅生成基础设施，不生成用户代码脚手架
actr gen --no-scaffold
```

`actr gen` 是连接 `.proto` 定义与具体编程语言实现的桥梁。它负责调用 `protoc` 编译器及 `actr-framework` 插件，生成类型安全的通信代码和业务逻辑框架。

**核心功能**：
1.  **基础设施生成**：生成 Protobuf 消息定义 (`struct`) 和 Actor 框架代码 (`trait`, `routing`, `client`)。
2.  **用户脚手架**：首次运行时，自动在源码目录（如 `src/`）生成业务逻辑实现模板（`*_service.rs`），包含详细的 TODO 注释引导开发。
3.  **代码格式化**：生成后自动调用对应语言的格式化工具（如 `rustfmt`）。
4.  **多语言支持**：
    - **Rust**: 默认。生成至 `src/generated`。
    - **Swift**: 生成至 `{PascalName}/Generated`。
    - **Kotlin**: 生成至 `app/src/main/java/.../generated`。
    - **Python**: 生成至 `generated`。
...
```bash
# 添加服务依赖（如果存在多个匹配项或版本，将显示交互式选择列表；如果是唯一匹配则自动安装）
actr install user-service

# 添加依赖并指定别名（推荐用于明确依赖用途）
actr install my_api --actr-type acme+user-service

# 直接指定 fingerprint（用于精确锁定，必须同时指定 actr-type）
actr install user-service --actr-type acme+user-service --fingerprint service_semantic:abc123...
```
...
1.  **空声明 + 交互式/自动选择**（推荐）:
    ...
    actr install user-service
    ```
    CLI 会搜索匹配 `user-service` 的所有服务：
    - 如果只有一个匹配项，自动安装。
    - 如果有多个（如不同版本或不同厂商），显示交互式列表供选择。
...
3.  **直接指定 fingerprint**（用于精确锁定）:
    **注意**：指定指纹时必须同时提供 `--actr-type` 以确保校验的准确性。
    ```bash
    actr install user-service --actr-type acme+user-service --fingerprint service_semantic:abc123...
    ```
    这会生成：
    ```toml
    [dependencies]
    user-service = { actr_type = "acme+user-service", fingerprint = "service_semantic:abc123..." }
    ```
这种设计确保了依赖的可重现性和安全性。
...
actr config set deployment.realm_id 1
actr config set discovery.visible true
...
# 查看所有配置（支持多种格式）
actr config show
actr config show --format json
actr config show --format yaml

# 删除配置
actr config unset signaling.url
...
**Package 配置（必填）**:
- `package.name` - 项目名称
- `package.description` - 项目描述（可选）
- `package.actr_type.manufacturer` - ActrType 制造商
- `package.actr_type.name` - ActrType 名称

**System 配置**:
- `signaling.url` / `system.signaling.url` - 信令服务器地址
- `deployment.realm_id` / `system.deployment.realm_id` - 部署 Realm ID
- `discovery.visible` / `system.discovery.visible` - 是否在服务发现中可见
- `storage.mailbox_path` / `system.storage.mailbox_path` - 邮箱存储路径

**WebRTC 配置**:
- `webrtc.stun_urls` / `system.webrtc.stun_urls` - STUN 服务器列表（逗号分隔）
- `webrtc.turn_urls` / `system.webrtc.turn_urls` - TURN 服务器列表（逗号分隔）
- `webrtc.force_relay` / `system.webrtc.force_relay` - 是否强制使用 TURN 中继

**Observability 配置**:
- `observability.filter_level` / `system.observability.filter_level` - 日志过滤级别
- `observability.tracing_enabled` / `system.observability.tracing_enabled` - 是否启用追踪
- `observability.tracing_endpoint` / `system.observability.tracing_endpoint` - 追踪端点
- `observability.tracing_service_name` / `system.observability.tracing_service_name` - 追踪服务名

**Scripts 配置**:
- `scripts.<name>` - 自定义脚本命令

##### **`actr config test` 子命令**
...
1. **语法检查**：解析 TOML 文件格式
2. **结构验证**：验证必填字段和数据类型
3. **配置摘要**：显示配置概览（Package、ActrType、Realm、Dependencies 等）
...
# 运行默认脚本（默认查找名为 "run" 的脚本）
actr run
```

`actr run` 命令提供了一种统一的方式来执行项目中定义的各种任务。脚本定义在 `Actr.toml` 的 `[scripts]` 部分。
...
# 开发脚本 (对应 `actr run dev`)
dev = "cargo watch -x run"

# 测试脚本 (对应 `actr run test`)
test = "cargo test"
...
**特性**:
- **统一入口**: 无论底层使用 Rust, Python 还是 Swift，开发者都可以使用统一的 `actr run dev` 命令启动开发环境。
- **智能提示**: 如果指定的脚本不存在，actr 会列出当前可用的所有脚本供参考。
- **跨平台**: 脚本命令在当前 shell 环境中执行，支持系统级命令。
...
```mermaid
graph TD
    A[protoc] --> B[解析 .proto 文件]
    B --> C[生成 FileDescriptorSet]
    C --> D[protoc-gen-actorframework]
    D --> E[分析 service 和 message]
    E --> F[生成服务 Handler Trait]
    F --> G[生成 MessageHandler blanket impl]
    G --> H[生成 Workload blanket impl]
    H --> I[生成 WorkloadRouting ZST]
    I --> J[生成 Context 扩展方法]
    J --> K[输出 Rust 代码]
```
