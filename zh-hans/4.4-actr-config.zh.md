...
pub struct ProtoFileWithContent {
    pub name: String,                // Proto 文件名: URI (actr://...) 或路径（如 "actr://101:acme+user-service@v1/user.v1.proto"）
...
[[dependency.files]]
name = "actr://101:acme+user-service@v1/user.v1.proto"       # Proto 文件名
...
[[dependency.files]]
name = "actr://101:acme+user-service@v1/common.v1.proto"
...
[[dependency.files]]
name = "actr://101:acme+payment-service@v1/payment.v2.proto"
...
- `uri` 使用 actr URI 格式（`actr://realm:manufacturer+name@v1/path.proto`）
...
lock_file.dependencies.push(LockedDependency {
    ...
    spec: ServiceSpecMeta {
        ...
        protobufs: vec![
            ProtoFileWithContent {
                name: "actr://101:acme+user-service@v1/user.v1.proto".to_string(),
