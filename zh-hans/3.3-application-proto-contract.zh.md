...
graph TD
    ...
    B["<b>2. 构建 & 生成代码</b><br/><i>(actr build)</i>"]
...
这个信息由构建工具（如 `actr`）在调用 `protoc` 编译器时提供。凡是直接传递给编译命令的文件，就是“主要目标”；通过 `import` 间接引入的，就是“依赖项”。
...
**做法**: 在 `Actr.toml` 的 `[dependencies]` 中声明依赖，并为其提供一个别名。`actr gen` 会为该别名生成一个专属的客户端，然后在代码中通过它来调用。
    ```rust
