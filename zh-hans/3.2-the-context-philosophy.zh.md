...
##### **MessageEnvelope 结构（完整参数说明）**

所有底层消息发送都需要手工构造 `MessageEnvelope`：
...
use actr_protocol::{ActrId, MessageEnvelope, MetadataEntry};
use prost::Message;
use uuid::Uuid;
...
*   **生成客户端**: `actr-cli gen` 会读取上述配置，并为 `Context` 生成一个名为 `user_api` 的方法，该方法返回一个包含了 `user-service` 所有 RPC 的客户端。
