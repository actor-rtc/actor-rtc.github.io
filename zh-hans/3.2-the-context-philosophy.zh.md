...
*   **请求ID**: `ctx.request_id()` 返回当前请求的唯一标识，用于关联请求和响应。
*   **分布式追踪**: 追踪上下文通过 OpenTelemetry 和 W3C Trace Context 标准自动传播，无需手动管理。系统会自动在 RPC 调用中注入和提取 `traceparent`/`tracestate` 头。
...
```rust
/// Context - Actor 执行上下文接口
/// 
/// 注意：Context 是一个 trait，由 Runtime 实现（RuntimeContext）。
/// Framework 只定义接口，实现依赖倒置。
#[async_trait]
...
pub struct RpcEnvelope {
    ...
    /// let payload = Some(req.encode_to_vec().into());
    /// ```
    ///
    /// 注意：
    ...
    ///
    /// 字段说明：
    /// - traceparent: 包含 trace_id、span_id 和采样标志
    /// - tracestate: 支持多供应商追踪系统协作
    ///
    /// 注意：
    /// - 系统会自动注入和提取这些字段
    /// - 开发者无需手动设置（启用 opentelemetry feature 时）
...
async fn manual_call_example(ctx: &Context, target_id: ActrId) -> ActorResult<GetUserResponse> {
    ...
    let envelope = RpcEnvelope {
        ...
        payload,
        traceparent: None,  // 自动注入（启用 opentelemetry feature 时）
        tracestate: None,   // 自动注入（启用 opentelemetry feature 时）
