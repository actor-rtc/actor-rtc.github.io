...
impl<T: EchoServiceHandler> Workload for EchoServiceWorkload<T> {
    ...
    fn actor_type(&self) -> ActrType {
        ActrType {
            ...
            name: "echo.v1.EchoService".to_string(),
        }
    }
}
```
...
sequenceDiagram
    ...
    Note over AS: 创建 Arc<MyEchoService>

    AS->>WR: configure(workload, registry)
    Note over WR: 静态方法调用

    WR->>HR: register<EchoRequest>(workload)
    Note over HR: 存储 TypeId → Handler 映射

    WR->>HR: register<BatchRequest>(workload)
    Note over HR: 存储更多映射

    HR-->>AS: 注册完成
    AS-->>User: ActrSystem (已组装)

    Note over AS,HR: 完成平行组合<br/>ActrSystem + Workload → ActrNode
```
...
| **Workload** | 工作单元身份 + 关联 Dispatcher | 提供 actor_type 和关联的 Dispatcher 类型 |
