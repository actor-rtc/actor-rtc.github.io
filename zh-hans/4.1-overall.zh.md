...
| Crate | 职责 | 依赖 |
|-------|------|------|
| **actr-cli** | 项目管理：`init`, `install`, `gen`, `run`<br/>代码生成：调用 `protoc-gen-actorframework`<br/>依赖管理：生成 `actr.lock.toml` | actr-config<br/>actr-version<br/>actr-protocol<br/>actr-framework |
